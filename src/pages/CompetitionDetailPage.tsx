    ...
    >
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-gray-800 bg-opacity-70 rounded-lg shadow-xl p-8 md:p-12">
          ...
          <p className="text-lg text-gray-200 mb-6 text-center md:text-left">Level: <span className="capitalize">{competition.level.replace('_', ' ')}</span></p>
          ...
          <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="md:w-1/2 flex flex-col items-center">
                ...
                <div className="w-full text-center">
                  {!user ? (
                    ...
                    <p className="text-gray-400">This competition is currently {competition.status}.</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="md:w-1/2 flex flex-col justify-between">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-gray-200">
                <div>
                  <h2 className="font-semibold text-xl mb-2 text-white">Eligibility Criteria</h2>
                  <p>{competition.eligibilityCriteria}</p>
                </div>
                <div>
                  <h2 className="font-semibold text-xl mb-2 text-white">Important Dates</h2>
                ...
                <div>
                  <h2 className="font-semibold text-xl mb-2 text-white">Judging Criteria</h2>
                  <p>{competition.judgingCriteria}</p>
                </div>
                <div>
                  <h2 className="font-semibold text-xl mb-2 text-white">Rewards</h2>
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-75 overflow-y-auto h-full w-full flex justify-center items-center">
          <div className="relative p-8 w-96 shadow-lg rounded-md text-center bg-gray-700">
            ...
            {!userSubmission ? (
              ...
              <>
                <p className="text-green-400 font-semibold mb-4">You have successfully applied to this competition!</p>
                <p className="text-gray-200 mb-2">Your application status: <span className="capitalize">{userSubmission.status.replace('_', ' ')}</span></p>
                {userSubmission.score && <p className="text-gray-200 mb-2">Score: {userSubmission.score}</p>}
                {userSubmission.feedback && <p className="text-gray-200 mb-4">Feedback: {userSubmission.feedback}</p>}
                <p className="text-gray-200 text-sm">Applied on: {new Date(userSubmission.submissionDate).toLocaleDateString()}</p>
